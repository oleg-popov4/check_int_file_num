...
def inizialisiere_ui():
    #Lade GUI from ui File
    ui = uic.loadUi('file_ui.ui')
...
def clicked_Button_directory(ui):
    global VERZEICHNISS, CHOIS, ALL_FILES, ALL_FOLDER, ALL_DATA
    verzeichniss = QFileDialog.getExistingDirectory()
...
def inizialisiere_button(ui):
    ...
    ui.buttonBox_start.accepted.connect(lambda: programm_start(ui))
    ui.buttonBox_start.rejected.connect(lambda: print('Programm Abbrechen'))
#End inizialisiere_button

if __name__ == '__main__':
    os.chdir(r'E:\Парсинг_на_Python\Zusatz_Programme\file_num_ui')
